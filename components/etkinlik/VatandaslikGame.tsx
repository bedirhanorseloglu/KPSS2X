...
import { VATANDASLIK_QUESTIONS, VatandaslikQuestion, VATANDASLIK_SECTIONS } from "@/lib/vatandaslikData";
...
export default function VatandaslikGame({ onComplete, onBack }: Props) {
  const [selectedSection, setSelectedSection] = useState<string>("Tümü");
  const [questions, setQuestions] = useState<VatandaslikQuestion[]>([]);
  ...
  useEffect(() => {
    startNewGame(selectedSection);
  }, [selectedSection]);

  const startNewGame = (sec: string) => {
    let filtered = VATANDASLIK_QUESTIONS;
    if (sec !== "Tümü") {
      filtered = VATANDASLIK_QUESTIONS.filter((q) => q.section === sec);
    }
      ...
      >
        ...
        <div>
          <h2 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight">
            Tebrikler! Vatandaşlık Testini Tamamladın
          </h2>
          <p className="text-sm font-bold text-slate-500 dark:text-slate-400 mt-1">
            KPSS Vatandaşlık & Anayasa Hukuku Soru Bankası Sonucu
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full pt-4">
          <button
            type="button"
            onClick={() => startNewGame(selectedSection)}
  ...
  return (
    <div className="max-w-3xl mx-auto w-full space-y-5 pb-44">
      ...
      {/* ━━━ SECTION SELECTOR PILLS (SCROLLABLE) ━━━ */}
      <div className="flex p-1.5 bg-slate-100 dark:bg-slate-900 rounded-2xl border-2 border-b-4 border-slate-200 dark:border-slate-800 w-full shadow-2xs gap-1.5 overflow-x-auto no-scrollbar scroll-smooth touch-pan-x flex-nowrap">
        {VATANDASLIK_SECTIONS.map((sec) => {
          const isActive = selectedSection === sec;
          const count = sec === "Tümü" 
            ? VATANDASLIK_QUESTIONS.length 
            : VATANDASLIK_QUESTIONS.filter((q) => q.section === sec).length;
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl font-black text-xs transition-all cursor-pointer whitespace-nowrap border-2 border-b-4 shrink-0 ${
                isActive
      ...
      {currentQ && (
        ...
        >
          ...
          {/* Question Text */}
          <h3 className="text-xl sm:text-2xl font-black text-slate-800 dark:text-white leading-relaxed tracking-tight whitespace-pre-line">
            {currentQ.question}
          </h3>
