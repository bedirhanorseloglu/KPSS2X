...
import { ArrowLeft, Map, ArrowRight, Mountain, Droplets, Layers } from "lucide-react";
  ...
  if (topicId && topic) {
    return (
      <main className="min-h-[100dvh] bg-white dark:bg-[#0f172a] pt-24 pb-8 px-2 lg:px-4 flex flex-col">
        <FlashcardGame topic={topic} onQuit={() => router.push("/etkinlik/kart")} />
      </main>
    );
  }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          <motion.div
            ...
            className="w-24 h-24 bg-[#ce82ff] rounded-[2rem] flex items-center justify-center mx-auto mb-6 shadow-[0_6px_0_0_#b16be0]"
          >
          ...
          <p className="text-lg text-slate-500 font-bold max-w-xl mx-auto leading-relaxed">
            Hızlı tekrar yöntemiyle coğrafi kavramları ve dağ/göl türlerini eğlenerek ezberleyin.
          </p>
        </motion.div>
                ...
                >
                  <div className="flex items-start gap-5 mb-6">
                    <div className={`w-20 h-20 rounded-[1.5rem] ${colors.bg} text-white shadow-sm flex items-center justify-center shrink-0 border-b-[6px] ${colors.border}`}>
                      {TOPIC_ICONS[t.id] ?? <Layers className="w-10 h-10" />}
                    </div>
                    <div>
    ...
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-[#0f172a]">
        <div className="flex flex-col items-center gap-4">
          <div className="w-16 h-16 rounded-[2rem] bg-[#ce82ff] border-b-[6px] border-[#b16be0] flex items-center justify-center animate-bounce">
